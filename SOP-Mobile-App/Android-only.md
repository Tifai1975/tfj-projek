# SOP Mobile App Development (Android Only)

## Tujuan
Memastikan pengembangan aplikasi Android (Kotlin) mengikuti prosedur terstruktur, terukur, dan tervalidasi secara mendalam. Semua tugas, tanggung jawab, dan checkpoint disetujui oleh CEO sebelum deploy.

## Lingkup
Menghubungkan semua aktivitas dari pengumpulan kebutuhan hingga deploy aplikasi Android (Kotlin).

## Definisi Peran (RACI)

| Aktivitas | CEO | PM | Android (Kotlin) | QA | Designer |
|-----------|-----|----|---------------------|----|----------|
...
| Pengembangan (Android) | I | A | R | C | C |
...
1. **Pertemuan Kickoff** — CEO, PM, Android Lead, QA Lead, Designer.
...
### 2.1 Desain UI/UX (Android)
- Mengikuti Material Design 3.
- Fokus: bottom navigation, gesture patterns, system UI.
- **Output**: **DESIGN.md** — design system (colors, typography, spacing) + platform-specific mockups.

### 2.2 Perancangan Teknis (Android)
- **API Contract**: OpenAPI/Swagger (Kotlin/Java).
- **Database**: Room / Hibernate.
...
- Environment: dev, staging, production (Android).
...
- **Accessibility**: WCAG 2.1 AA (fokus TalkBack).
- **Compatibility**: Uji di Android 14+ (versi minimal).

### 5.3 Output: Test Report
- **test-report-Android.md** — daftar bug (seperti SOP website) dengan status (open/resolved).

## Fase 6: Review & Persetujuan Final
1. **Pertemuan Review** (PM + Android Lead + QA + CEO).
2. **Kriteria Keluar**:
...
- **Production**: Deploy ke Google Play Store.
...
| `DESIGN.md` | Design system + Android-specific mockups |
| `TECH.md` | API spec, DB schema, architecture diagram |
| `RACI.md` | Matriks tanggung jawab (RACI) |
| `test-report-Android.md` | Laporan pengujian Android |
...
---
*Disusun untuk SOP Mobile App Development (Android Only)*
*Versi: 1.0*
*Tanggal: 2026-09-09*